/* [ ---- Beoro Admin - invoices ---- ] */
    ...
    $(document).ready(function() {
       ...
       //* subtotal sum
        $('#inv_form').on('keyup','.jQinv_item_crdhurs, .jQinv_item_totalcrdhrs, .jQinv_item_cgp, .jQinv_item_cgp1, .jQinv_item_cgp2, .jQinv_item_cgp3, .jQinv_item_cgp4', function() {
            rowInputs();
        });
        ...
        function rowInputs() {
            ...
            $(".invE_table tr").not('.last_row').each(function () {
				...
				var $lineitemtotal 	= (($('.jQinv_item_cgp', this).val() * 1) + ($('.jQinv_item_cgp1', this).val() * 1) + ($('.jQinv_item_cgp2', this).val() * 1) + ($('.jQinv_item_cgp3', this).val() * 1) + ($('.jQinv_item_cgp4', this).val() * 1)); 
