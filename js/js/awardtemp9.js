/* [ ---- Beoro Admin - invoices ---- ] */
    ...
    $(document).ready(function() {
       ...
       //* subtotal sum
        $('#inv_form').on('keyup','.jQinv_item_crdhurs, .jQinv_item_totalcrdhrs, .jQinv_item_cgp, .jQinv_item_cgp1, .jQinv_item_cgp2, .jQinv_item_cgp3, .jQinv_item_cgp4, .jQinv_item_cgp5, .jQinv_item_cgp6, .jQinv_item_cgp7, .jQinv_item_cgp8', function() {
            rowInputs();
        });
        ...
        function rowInputs() {
            ...
            $(".invE_table tr").not('.last_row').each(function () {
				...
				var $lineitemtotal 	= (($('.jQinv_item_cgp', this).val() * 1) + ($('.jQinv_item_cgp1', this).val() * 1) + ($('.jQinv_item_cgp2', this).val() * 1) + ($('.jQinv_item_cgp3', this).val() * 1) + ($('.jQinv_item_cgp4', this).val() * 1) + ($('.jQinv_item_cgp5', this).val() * 1) + ($('.jQinv_item_cgp6', this).val() * 1) + ($('.jQinv_item_cgp7', this).val() * 1)+ ($('.jQinv_item_cgp8', this).val() * 1));  
