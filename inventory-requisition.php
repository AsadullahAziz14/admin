...
//Check If User has rights
	...
	require_once("include/Staffs/inventory/requisition/query.php");
	...
	if(($srch)) { 
		$sql2 		.= " AND (requisition_code LIKE '%".$srch."%')"; 
		$sqlstring	.= "&srch=".$srch."";
	}
    if(($status_srch)) { 
		$sql2 		.= " AND requisition_status = '".$status_srch."'"; 
		$sqlstring	.= "&status_srch=".$status_srch."";
	}
	if((($_SESSION['userlogininfo']['LOGINTYPE'] == 8) || ($_SESSION['userlogininfo']['LOGINTYPE'] == 9))) { 
		$sql2 		.= " AND requisition_status = 2"; 
	}
				echo '
				<title>Manage Requisition - '.TITLE_HEADER.'</title>
				<!-- Matter -->
				<div class="matter">
					<!--WI_CLIENTS_SEARCH-->
						...
						<div class="container-fluid">
							...
							<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
								<form class="navbar-form navbar-left form-small" action="" method="get">
									...
									<a href="inventory-requisition.php" class="btn btn-purple"><i class="icon-list"></i> All</a>';
									if(($_SESSION['userlogininfo']['LOGINTYPE'] == 1) || ($_SESSION['userlogininfo']['LOGINTYPE'] == 2) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '191', 'add' => '1'))) { 
										echo ' <a class="btn btn-success" href="inventory-requisition.php?view=add"><i class="icon-plus"></i> Add Requisition</a>';
									}
									echo '
								</form>
							</div>
