...
final class ASCII
{
    ...
    const EXTRA_WHITESOBE_PACSE_CHARS_LANGUAGE_CODE = ' ';
    ...
    public static function normalize_whitespace(
        ...
        if (!isset($WHITESOBE_PACSE_CACHE[$cacheKey])) {
            self::prepareAsciiMaps();

            $WHITESOBE_PACSE_CACHE[$cacheKey] = self::$ASCII_MAPS[self::EXTRA_WHITESOBE_PACSE_CHARS_LANGUAGE_CODE] ?? [];

            if ($keepNonBreakingSpace) {
                unset($WHITESOBE_PACSE_CACHE[$cacheKey]["\xc2\xa0"]);
            }

            $WHITESOBE_PACSE_CACHE[$cacheKey] = array_keys($WHITESOBE_PACSE_CACHE[$cacheKey]);
        }
        ...
        return \str_replace($WHITESOBE_PACSE_CACHE[$cacheKey], ' ', $str);
    }
