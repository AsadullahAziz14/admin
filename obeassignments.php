...
{   
	...
	if(($srch)) { 
		$sql2 		.= " AND (".OBE_ASSIGNMENTS.".domain_name LIKE '%".$srch."%')"; 
		$sqlstring	.= "&srch=".$srch."";
	}
	if(($domain_level_name_srch)) { 
		$sql2 		.= " AND ".OBE_ASSIGNMENTS.".domain_level_name = '".$domain_level_name_srch."'"; 
		$sqlstring	.= "&domain_level_name_srch=".$domain_level_name_srch."";
	}
    if(($status_srch)) { 
		$sql2 		.= " AND ".OBE_ASSIGNMENTS.".domain_level_status = '".$status_srch."'"; 
		$sqlstring	.= "&status_srch=".$status_srch."";
	}

	echo '
	<title>Manage Assignments - '.TITLE_HEADER.'</title>
	<!-- Matter -->
	<div class="matter">
	<!--WI_CLIENTS_SEARCH-->
	...
	<form class="navbar-form navbar-left form-small" action="" method="get">
		...
		<a href="obeassignments.php" class="btn btn-purple"><i class="icon-list"></i> All</a>';
		// if((($_SESSION['userlogininfo']['LOGINTYPE'] == 1) || ($_SESSION['userlogininfo']['LOGINTYPE'] == 2) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '19', 'add' => '1')))) { 
			echo ' <a data-toggle="modal" class="btn btn-success" href="obeassignments.php?view=add"><i class="icon-plus"></i> Add Assignment</a>';
		// }
	echo '
	</form>
	</div>
