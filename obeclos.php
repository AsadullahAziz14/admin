...
{   
	...
	if(($srch)) { 
		$sql2 		.= " AND (cl.clo_statement LIKE '%".$srch."%')"; 
		$sqlstring	.= "&srch=".$srch."";
	}
    if(($status_srch)) { 
		$sql2 		.= " AND cl.clo_status = '".$status_srch."'"; 
		$sqlstring	.= "&status_srch=".$status_srch."";
	}

	echo '
	<title>Manage CLOs - '.TITLE_HEADER.'</title>
	<!-- Matter -->
	<div class="matter">
	<!--WI_CLIENTS_SEARCH-->
	...
	<form class="navbar-form navbar-left form-small" action="" method="get">
		<div class="form-group">
			<input type="text" class="form-control" name="srch" placeholder="CLO Statement" style="width:250px;">
		</div>
		<div class="form-group">
		...
		<a href="obeclos.php" class="btn btn-purple"><i class="icon-list"></i> All</a>';
		if(($_SESSION['userlogininfo']['LOGINTYPE'] == 1) || ($_SESSION['userlogininfo']['LOGINTYPE'] == 2) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '19', 'add' => '1'))) { 
			echo ' <button data-toggle="modal" class="btn btn-success" href="#addNewCLOModal"><i class="icon-plus"></i>Add CLO</button>';
		}
	echo '
	</form>
	</div>
