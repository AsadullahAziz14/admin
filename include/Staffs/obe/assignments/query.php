<?php
// Assignment Crud Start
if(isset($_GET['deleteassignmentId'])) {
    $resultrecordId = $_GET['deleteassignmentId'];
    $assignmentrecordId = $_GET['deleteassignmentId'];
   
    $sqllms = $dblms->querylms("SELECT id_ques 
                                    FROM ".OBE_ASSIGNMENTS." 
                                    WHERE assignment_id =  ".cleanvars($_GET['deleteassignmentId'])." ");
    $value_assignment = mysqli_fetch_assoc($sqllms);

    if($value_assignment['id_ques'] != '') {        
        $deleteQuessqllms = $dblms->querylms("DELETE FROM ".OBE_QUESTIONS." WHERE ques_id IN (".$value_assignment['id_ques'].")");
        $deleteMcqsqllms = $dblms->querylms("DELETE FROM ".OBE_MCQS." WHERE id_ques IN (".$value_assignment['id_ques'].")");
        $sqllms = $dblms->querylms("SELECT DISTINCT id_result as resultId 
                                                        FROM ".OBE_QUESTIONS_RESULTS." 
                                                        WHERE id_ques IN (".$value_assignment['id_ques'].")");
        $row = mysqli_fetch_array($sqllms);
        $deleteQuestionResultsqllms = $dblms->querylms("DELETE FROM ".OBE_QUESTIONS_RESULTS." WHERE id_ques IN (".$value_assignment['id_ques'].") AND id_result IN (".$row['resultId'].")");
        $deleteResultsqllms = $dblms->querylms("DELETE FROM ".OBE_RESULTS." WHERE result_id IN (".$row['resultId'].")");
    }
    $deleteAssignmentsqllms = $dblms->querylms("DELETE FROM ".OBE_ASSIGNMENTS." WHERE assignment_id = '".cleanvars($_GET['deleteassignmentId'])."'");

    if($deleteAssignmentsqllms) {
...
if(isset($_POST['questionId'])) {
    ...
    $sqllms = $dblms->querylms("UPDATE ".OBE_ASSIGNMENTS." SET id_ques = TRIM(BOTH '' FROM REPLACE(REPLACE(id_ques, '".cleanvars($_POST['questionId'])."'''), ',', '')) WHERE id_ques LIKE '%".cleanvars($_POST['questionId'])."%'");
}
...
if (isset($_POST['submit_assignment'])) {
    ...
    $assignmentData = [
        'assignment_status'            => cleanvars($_POST['assignment_status'])                    ,
        'assignment_number'            => cleanvars($_POST['assignment_number'])                    ,
        'assignment_marks'             => cleanvars($_POST['marks'])                                ,
        'assignment_date'              => cleanvars($_POST['assignment_date'])                      ,
        'id_ques'                      => $quesids                                                  ,
        'id_teacher'                   => ID_TEACHER                                                ,
        'id_course'                    => ID_COURSE                                                 ,
        'id_prg'                       => ID_PRG                                                    ,
    ...
    $queryInsertAssignment = $dblms->Insert(OBE_ASSIGNMENTS, $assignmentData);

    if($queryInsertAssignment) { 
...
if(isset($_POST['save_changes'])) {
    $assignment_id = cleanvars($_GET['id']); 
    $ques_ids = [];
    ...
    foreach (cleanvars($_POST['ques_number']) as $key => $value) {        
        if(is_array($value)) {
            ...
            if($queryUpdateQues && cleanvars($_POST['ques_category'][$key][$val[0]]) == "2" && cleanvars($_POST['option'])) {
                $sqllmsmcq = $dblms->querylms("SELECT id_ques FROM ".OBE_MCQS." WHERE id_ques = ".$val[0]);
    ...
    $conditions = "WHERE assignment_id = ".cleanvars($_GET['id'])."";
    $queryUpdateAssignment = $dblms->update(OBE_ASSIGNMENTS, $update_assignmentData, $conditions);

    if($queryUpdateAssignment) {
...
if(isset($_POST['edit_result'])) {
    ...
    foreach (cleanvars($_POST['obt_marks']) as $stdid => $questions) {
        foreach ($questions as $quesid => $obtmarks) {
            ...
            $conditions = "WHERE id_ques = ".$quesid."AND id_std = ".$stdid."AND id_result = ".cleanvars($_POST['result_id'])." ";
...
if(isset($_GET['deleteresultId'])) {
    ...
    $sqllms = $dblms->querylms("SELECT id_ques 
                                    FROM ".OBE_ASSIGNMENTS." 
                                    WHERE assignment_id =  ".$resultrecordId."
                                ");
    
    if(mysqli_num_rows($sqllms) > 0) {
        $value_assignment = mysqli_fetch_array($sqllms);
        if($value_assignment['id_ques'] != "") {
