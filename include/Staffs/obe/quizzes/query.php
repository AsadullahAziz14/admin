<?php
// Quiz Crud Start
if (isset($_GET['deletequizId'])) {
    $resultrecordId     = $_GET['deletequizId'];
    $quizrecordId       = $_GET['deletequizId'];

    $sqllms = $dblms->querylms("SELECT id_ques 
                                    FROM ".OBE_QUIZZES." 
                                    WHERE quiz_id =  ".cleanvars($_GET['deletequizId'])."
                                    ");
    $value_quiz = mysqli_fetch_assoc($sqllms);

    if ($value_quiz['id_ques'] != '') {
        $deleteQuessqllms               = $dblms->querylms("DELETE FROM ".OBE_QUESTIONS." WHERE ques_id IN (".$value_quiz['id_ques'].")");
        $deleteMcqsqllms                = $dblms->querylms("DELETE FROM ".OBE_MCQS." WHERE id_ques IN (".$value_quiz['id_ques'].")");
        $sqllms                         = $dblms->querylms("SELECT DISTINCT id_result as resultId 
                                                                FROM ".OBE_QUESTIONS_RESULTS." 
                                                                WHERE id_ques IN (".$value_quiz['id_ques'].")");
        $row                            = mysqli_fetch_array($sqllms);
        $deleteQuestionResultsqllms     = $dblms->querylms("DELETE FROM ".OBE_QUESTIONS_RESULTS." WHERE id_ques IN (".$value_quiz['id_ques'].") && id_result IN (".$row['resultId'].")");
        $deleteResultsqllms             = $dblms->querylms("DELETE FROM ".OBE_RESULTS." WHERE result_id IN (".$row['resultId'].")");
    }

    $deleteQuizsqllms = $dblms->querylms("DELETE FROM ".OBE_QUIZZES." WHERE quiz_id = '".cleanvars($_GET['deletequizId'])."'");
...
if (isset($_POST['questionId'])) {
    ...
    $sqllms             = $dblms->querylms("UPDATE ".OBE_QUIZZES." SET id_ques = TRIM(BOTH ',' FROM REPLACE(REPLACE(id_ques, '".cleanvars($_POST['questionId'])."',''), ',,', ',')) WHERE id_ques LIKE '%".cleanvars($_POST['questionId'])."%'");
}
...
if (isset($_POST['submit_quiz'])) {
    ...
    $quizData = [
        ...
        'quiz_number'           => cleanvars($_POST['quiz_number'])                     ,
        'quiz_marks'            => cleanvars($_POST['marks'])                           ,
        'quiz_date'             => cleanvars($_POST['quiz_date'])                       ,
        'id_ques'               => $quesids                                             ,
        'id_teacher'            => ID_TEACHER                                           ,
        'id_course'             => ID_COURSE                                            ,
        'id_prg'                => ID_PRG                                               ,
...
if (isset($_POST['save_changes'])) {
    ...
    foreach (cleanvars($_POST['ques_number']) as $key => $value) {
        if (is_array($value)) {
            ...
            if ($queryUpdateQues && cleanvars($_POST['ques_category'][$key][$val[0]]) == "2" && cleanvars($_POST['option'])) {
                $queryMCQs = $dblms->querylms("SELECT id_ques 
                                                FROM ".OBE_MCQS." 
                                                WHERE id_ques = ".$val[0]);
                $valueMCQs = mysqli_fetch_assoc($queryMCQs);

                if (isset($valueMCQs['id_ques'])) {
                    $mcqData = [
                ...
                $queryMCQs = $dblms->querylms("DELETE FROM ".OBE_MCQS." WHERE id_ques IN (SELECT id_ques FROM ".OBE_MCQS." WHERE id_ques = ".$val[0].")");
            }
        } else {
            $quesData = [
                'ques_status'                   => '1'                                                  ,
    ...
    $update_quizData = [
        'quiz_status'                   => cleanvars($_POST['quiz_status'])                     ,
        'quiz_number'                   => cleanvars($_POST['number'])                          ,
        'quiz_marks'                    => cleanvars($_POST['marks'])                           ,
        'quiz_date'                     => cleanvars($_POST['date'])                            ,
    ...
    $queryUpdateQuiz = $dblms->update(OBE_QUIZZES, $update_quizData, $conditions);
...
if (isset($_GET['deleteresultId'])) {
    ...
    $sqllms = $dblms->querylms("SELECT id_ques 
                                    FROM ".OBE_QUIZZES." 
                                    WHERE quiz_id =  ".$resultrecordId."");

    if (mysqli_num_rows($sqllms) > 0) {
        $value_quiz = mysqli_fetch_array($sqllms);
        if ($value_quiz['id_ques'] != "") {
