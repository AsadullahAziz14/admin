	...
	if (LMS_VIEW == 'add' && !isset($_GET['id'])) {
		...
		<div class="row">
			<div class="modal-dialog" style="width:90%;">
				<form class="form-horizontal" action="obequizzes.php" method="POST" id="addNew" enctype="multipart/form-data">
					<div class="modal-content">
						<div class="modal-header">
							<h4 class="modal-title" style="font-weight:700;"> Add Quiz</h4>
						</div>
						<div class="modal-body">
							<div class="form-group col-sm-61">
								<label class="control-label req col-lg-12" style="width:auto;"> <b>Quiz Number</b></label>
								<div class="col-lg-12">';
								$queryQuiz = $dblms->querylms("SELECT max(quiz_number) as quiz_number 
																	FROM ".OBE_QUIZZES." 
								...
								if ($valueQuiz != "") {
									$quizNumber = $valueQuiz['quiz_number'] + 1;
								} else {
									$quizNumber = 0;
								}
								echo '
									<select id="quiz_number" name="quiz_number" style="width:100%" autocomplete="off" required>
										<option value="">Select Option</option>';
											for ($i = $quizNumber; $i <= 50; $i++) {
							...
							<div class="form-group col-sm-61">
								<label class="control-label req col-lg-12" style="width:auto;"> <b>Quiz Date</b></label>
								<div class="col-lg-12">
									<input type="date" class="form-control" id="quiz_date" name="quiz_date" required></input>
								</div>
							</div>
							<div class="form-group col-sm-61">
								<label class="control-label req col-lg-12" style="width:auto;"> <b>Quiz Marks</b></label>
								<div class="col-lg-12">
							...
							<div class="form-group col-sm-61">
								...
								<div class="col-lg-12">
									<select id="quiz_status" name="quiz_status" style="width:100%" autocomplete="off" required>
										<option value="">Select Option</option>';
										foreach ($status as $quiz_status) {
											echo '<option value="'.$quiz_status['id'].'">'.$quiz_status['name'].'</option>';
										}
										echo '
									</select>
								</div>
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-default" data-dismiss="modal" onclick="location.href=\'obequizzes.php\'">Close</button>
							<input class="btn btn-primary" type="submit" value="Add Record" id="submit_quiz" name="submit_quiz">
						</div>
					</div>
				</form>
			</div>
