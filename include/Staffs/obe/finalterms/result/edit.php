...
if (LMS_VIEW == 'editresult' && isset($_GET['id'])) {
   $sqllms  = $dblms->querylms("SELECT id_ques 
                              ...
                              ");
   $value_finaltermsqllms = mysqli_fetch_array($sqllms);

   if($value_finaltermsqllms['id_ques'] == "") {
      echo "
      <script>
      ...
      $quesResult_sqllms = $dblms->querylms("SELECT id_ques,id_result 
                                                ...
                                                WHERE id_ques IN (".$value_finaltermsqllms['id_ques'].")");
      if(mysqli_num_rows($quesResult_sqllms) <= 0) {
         ...
         $quessqllms = $dblms->querylms("SELECT ques_id, ques_number, ques_marks 
                                             FROM ".OBE_QUESTIONS." 
                                             WHERE ques_id IN (".$value_finaltermsqllms['id_ques'].") ORDER BY ques_number ASC");
         echo'
         <div class="container">
                     ...
                     <tbody>';
                        ...
                        foreach (STUDENT as $key => $value) {
                           ...
                           $resultsqllms = $dblms->querylms("SELECT qr.id_ques,qr.id_std, qr.obt_marks, qu.ques_id,qu.ques_number,qu.ques_marks
                                                               ...
                                                               WHERE qr.id_ques IN (".$value_finaltermsqllms['id_ques'].") AND qr.id_std IN (".$key.") ORDER BY qu.ques_number ASC;");
