<?php
if(isset($_GET['deleteMidtermId'])) {  
    $sqllms = $dblms->querylms("SELECT id_ques FROM ".OBE_MIDTERMS." WHERE mt_id =  ".cleanvars($_GET['deleteMidtermId'])."");
    $value_midterm = mysqli_fetch_assoc($sqllms);

    if($value_midterm['id_ques'] != '') {        
        $deleteQuessqllms = $dblms->querylms("DELETE FROM ".OBE_QUESTIONS." WHERE ques_id IN (".$value_midterm['id_ques'].")");
        $deleteMcqsqllms = $dblms->querylms("DELETE FROM ".OBE_MCQS." WHERE id_ques IN (".$value_midterm['id_ques'].")");
        $sqllms = $dblms->querylms("SELECT DISTINCT id_result as resultId FROM ".OBE_QUESTIONS_RESULTS." WHERE id_ques IN (".$value_midterm['id_ques'].")");
        $row = mysqli_fetch_array($sqllms);
        
        $deleteQuestionResultsqllms = $dblms->querylms("DELETE FROM ".OBE_QUESTIONS_RESULTS." WHERE id_ques IN (".$value_midterm['id_ques'].") && id_result IN (".$row['resultId'].")");
        $deleteResultsqllms = $dblms->querylms("DELETE FROM ".OBE_RESULTS." WHERE result_id IN (".$row['resultId'].")");
    }

    $deletemidtermsqllms = $dblms->querylms("DELETE FROM ".OBE_MIDTERMS." WHERE mt_id = '".cleanvars($_GET['deleteMidtermId'])."'");

    if($deletemidtermsqllms) {
...
if(isset($_POST['questionId'])) {
    ...
    $sqllms = $dblms->querylms("UPDATE ".OBE_MIDTERMS."SET id_ques = TRIM(BOTH ',' FROM REPLACE(REPLACE(id_ques, '".cleanvars($_POST['questionId'])."',''), ',,', ',')) WHERE id_ques LIKE '%".cleanvars($_POST['questionId'])."%'");
}

if (isset($_POST['submit_midterm'])) {
    $quesIds = array();
    ...
    $midtermData = [
        'mt_status'                    => cleanvars($_POST['mt_status'])                     ,
        'mt_number'                    => cleanvars($_POST['number'])                         ,
        'mt_marks'                     => cleanvars($_POST['marks'])                          ,
        'mt_date'                      => cleanvars($_POST['date'])                           ,
        'id_ques'                      => $quesids                                            ,
        'id_teacher'                   => ID_TEACHER                                          ,
        'id_course'                    => ID_COURSE                                           ,
        'id_prg'                       => ID_PRG                                              ,
    ...
    if($queryInsertMidterm) { 
        $_SESSION['msg']['status'] = '<div class="alert-box error"><span>Success: </span>Record has been Inserted successfully.</div>';
        header("Location: ".$_SERVER['HTTP_REFERER']."", true, 301);
        exit();
    }
}

if(isset($_POST['edit_midterm'])) {
    $mt_id = cleanvars($_GET['id']); 
    $ques_ids = [];
    ...
    $update_midtermData = [
        'mt_status'            => cleanvars($_POST['mt_status'])                        ,
        'mt_number'            => cleanvars($_POST['mt_number'])                        ,
        'mt_marks'             => cleanvars($_POST['marks'])                            ,
        'mt_date'              => cleanvars($_POST['mt_date'])                          ,
    ...
    $queryUpdateMidterm = $dblms->update(OBE_MIDTERMS, $update_midtermData, $conditions);

    if($queryUpdateMidterm) {
        $_SESSION['msg']['status'] = '<div class="alert-box error"><span>Success: </span>Record has been Updated successfully.</div>';
            header("Location: ".$_SERVER['HTTP_REFERER']."", true, 301);
            exit();
    }      
}
    ...
    if(mysqli_num_rows($sqllms) > 0) {
        ...
        if($value_midterm['id_ques'] != "") {
            $sqllms = $dblms->querylms("SELECT id_ques,id_result 
                                       ...
                                       WHERE id_ques IN (".$value_midterm['id_ques'].")");
            if(mysqli_num_rows($sqllms) > 0) {
