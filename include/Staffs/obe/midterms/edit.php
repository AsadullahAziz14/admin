...
if(($_SESSION['userlogininfo']['LOGINTYPE'] == 1) || ($_SESSION['userlogininfo']['LOGINTYPE'] == 2) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '19', 'edit' => '1'))) {
	if (!LMS_VIEW && isset($_GET['id'])) {
		$queryMidterm = $dblms->querylms("SELECT mt_id, mt_status, mt_number, mt_marks, mt_date, id_ques 
											FROM ".OBE_MIDTERMS." 
											WHERE mt_id = ".cleanvars($_GET['id'])." ");
		$valueMidterm = mysqli_fetch_assoc($queryMidterm);
		$formattedDate = date('Y-m-d', strtotime($valueMidterm['mt_date']));
		echo '
				...
				<form class="form-horizontal" action="obemidterms.php?id='.$_GET['id'].'" method="post" id="midtermForm" enctype="multipart/form-data">
					<div class="modal-content">
						<div class="modal-header">
							<h4 class="modal-title" style="font-weight:700;"> Edit MidTerm</h4>
						</div>
						<div class="modal-body">
							<div class="col-sm-61">
								<div class="form-sep" style="margin-top:5px;">
									<label for="mt_number" class="req"><b>Midterm Number</b></label>
									<input type="number" class="form-control" id="mt_number" name="mt_number" value="'.$valueMidterm['mt_number'].'" required>
								</div>
							</div>
							<div class="col-sm-61">
								<div class="form-sep" style="margin-top:5px;">
									<label for="mt_status" class="req" style="width:100px;"> <b>Status</b></label>';
									echo '
									<select id="mt_status" class="form-control" name="mt_status" style="width:100%" required>
										<option value="">Select Status</option>';
										foreach ($status as $midtermStatus) {
											if ($valueMidterm['mt_status'] == $midtermStatus['id']) {
												echo "<option value='$midtermStatus[id]' selected>$midtermStatus[name]</option>";
											} else {
							...
							<div class="col-sm-61">
								<div class="form-sep" style="margin-top:5px;">
									...
									<input type="text" class="form-control" id="marks" name="marks" value="'.$valueMidterm['mt_marks'].'" readonly required>
								</div>
							</div>                     
							<input type="hidden" class="form-control" id="questionId" name="questionId">
							<div class="col-sm-61">
								<div class="form-sep" style="margin-top:5px;">
									<label for="date" class="req"><b>MidTerm Date</b></label>
									<input type="date" class="form-control" id="mt_date" name="mt_date" value="'.$formattedDate.'" required>
								</div>
							</div>
							<div style="clear:both;"></div>
							<div id="questionContainer" style="margin-top:10px; margin-bottom: 10px;" class="col-sm-91">';
								if($valueMidterm['id_ques']) {
									$questionIds = explode(',', $valueMidterm['id_ques']);
						...
						<div class="modal-footer">
							...
							<input type="submit" name="edit_midterm" value="Update MidTerm" class="btn btn-primary">
						</div>
					</div>
				</form>
			</div>
