...
if(!LMS_VIEW && !isset($_GET['id'])) { 
   ...
   $page = (int)$page;
	...
	$queryIssuance = $dblms->querylms("SELECT issuance_id
											FROM ".SMS_ISSUANCE." 
											WHERE issuance_id != ''
										");
	$count 		= mysqli_num_rows($queryIssuance);
	...
	if(mysqli_num_rows($queryIssuance) > 0) {
		...
		if($count > $Limit) {
			...
			<div class="widget-foot">
			   ...
			   if($lastpage > 1) {	
				  ...
				  //previous button
				  if ($page > 1) {
					 $pagination.= '<li><a href="inventory-issuance.php?page='.$prev.$sqlstring.'">Prev</a></li>';
				  }
				  //pages	
				  if ($lastpage < 7 + ($adjacents * 3)) {	
					 ...
					 for ($counter = 1; $counter <= $lastpage; $counter++) {
						if ($counter == $page) {
						   ...
						   $pagination.= '<li><a href="inventory-issuance.php?page='.$counter.$sqlstring.'">'.$counter.'</a></li>';
						}
					 }
					 ...
					 if($page < 1 + ($adjacents * 3)) {
						...
						$pagination.= '<li><a href="inventory-issuance.php?page='.$lpm1.$sqlstring.'">'.$lpm1.'</a></li>';
						$pagination.= '<li><a href="inventory-issuance.php?page='.$lastpage.$sqlstring.'">'.$lastpage.'</a></li>';	
					 } else if($lastpage - ($adjacents * 3) > $page && $page > ($adjacents * 3)) { //in middle; hide some front and some back
						   $pagination.= '<li><a href="inventory-issuance.php?page=1'.$sqlstring.'">1</a></li>';
						   $pagination.= '<li><a href="inventory-issuance.php?page=2'.$sqlstring.'">2</a></li>';
						   $pagination.= '<li><a href="inventory-issuance.php?page=3'.$sqlstring.'">3</a></li>';
						   $pagination.= '<li><a href="#"> ... </a></li>';
			   ...
			   //next button
			   if ($page < $counter - 1) {
				  $pagination.= '<li><a href="inventory-issuance.php?page='.$next.$sqlstring.'">Next</a></li>';
			   } else {
				  $pagination.= "";
			   }
			   echo $pagination;
