...
if(!LMS_VIEW && !isset($_GET['id'])) { 
	...
	$queryInventory = $dblms->querylms("SELECT inventory_id
										FROM ".SMS_INVENTORY." 
										WHERE inventory_id != ''
									 ");
	$count 		= mysqli_num_rows($queryInventory);
	...
	if(mysqli_num_rows($queryInventory) > 0) {
		...
		if($count > $Limit) {
			...
			<div class="widget-foot">
			   ...
			   if($lastpage > 1) {	
				  ...
				  //previous button
				  if ($page > 1) {
					 $pagination.= '<li><a href="inventory-inventory.php?page='.$prev.$sqlstring.'">Prev</a></li>';
				  }
				  //pages	
				  if ($lastpage < 7 + ($adjacents * 3)) {	
					 ...
					 for ($counter = 1; $counter <= $lastpage; $counter++) {
						if ($counter == $page) {
						   ...
						   $pagination.= '<li><a href="inventory-inventory.php?page='.$counter.$sqlstring.'">'.$counter.'</a></li>';
						}
					 }
					 ...
					 if($page < 1 + ($adjacents * 3)) {
						...
						$pagination.= '<li><a href="inventory-inventory.php?page='.$lpm1.$sqlstring.'">'.$lpm1.'</a></li>';
						$pagination.= '<li><a href="inventory-inventory.php?page='.$lastpage.$sqlstring.'">'.$lastpage.'</a></li>';	
					 } else if($lastpage - ($adjacents * 3) > $page && $page > ($adjacents * 3)) { //in middle; hide some front and some back
						   $pagination.= '<li><a href="inventory-inventory.php?page=1'.$sqlstring.'">1</a></li>';
						   $pagination.= '<li><a href="inventory-inventory.php?page=2'.$sqlstring.'">2</a></li>';
						   $pagination.= '<li><a href="inventory-inventory.php?page=3'.$sqlstring.'">3</a></li>';
						   $pagination.= '<li><a href="#"> ... </a></li>';
			   ...
			   //next button
			   if ($page < $counter - 1) {
				  $pagination.= '<li><a href="inventory-inventory.php?page='.$next.$sqlstring.'">Next</a></li>';
			   } else {
				  $pagination.= "";
			   }
			   echo $pagination;
