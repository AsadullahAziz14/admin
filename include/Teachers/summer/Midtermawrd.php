...
if($valuemarks['id']) { 
	...
	$sqllms = $dblms->querylms("INSERT INTO ".SUMMER_MIDTERM." (
															status										,
															forward_to									,
															id_curs										,														
															timing										, 
...
//--------------------------------------
	$logremarks = 'Add Student Summer Mid Term Award List #: '.$idsetup.' for Course: '.$rowsurs['curs_code'].'-'.$rowsurs['curs_name'];
...
//------------------------------------------------
		$sqllmsmulti  = $dblms->querylms("INSERT INTO ".SUMMER_MIDTERM_DETAILS."( 
																				id_midterm									,
																				id_std										, 
																				marks										,
																				attendance								
...
//--------------------------------------
	$sqllms  = $dblms->querylms("UPDATE ".SUMMER_MIDTERM." SET status		= '1'
...
if($sqllms) {
//--------------------------------------
	$logremarks = 'Update Student Sumer Mid Term Award List #: '.$_POST['id_setup'].' for Course: '.$rowsurs['curs_code'].'-'.$rowsurs['curs_name'];
		$sqllmslog  = $dblms->querylms("INSERT INTO ".LOGS." (
															id_user										, 
															filename									, 
															action										,
...
//--------------------------------------
	$sqllmsdelte  = $dblms->querylms("DELETE FROM ".SUMMER_MIDTERM_DETAILS." WHERE id_midterm = '".$_POST['id_setup']."'");
//--------------------------------------
	$arraychecked = $_POST['id_std'];
//--------------------------------------
	for($ichk=1; $ichk<=sizeof($arraychecked); $ichk++){
...
//--------------------------------------------------
	$sqllmsstds  = $dblms->querylms("SELECT *      
										...
										AND ca.timing = '".cleanvars($_GET['timing'])."' 
...
$srbk++;
	$sqllmschecker  = $dblms->querylms("SELECT * 
												FROM ".SUMMER_MIDTERM_DETAILS." d 
												INNER JOIN ".SUMMER_MIDTERM." m ON m.id = d.id_midterm  
...
if($valuemarks['id_midterm']) {
	$printaward = '<a class="btn btn-mid btn-info" href="summermidtermawardprint.php?id='.$valuemarks['id'].'" target="_blank"><i class="icon-print"></i> Print Sheet</a>';
