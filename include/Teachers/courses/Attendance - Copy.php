...
<tr>
	...
	<td style="width:70px; text-align:center;">'.$rowLectures['TotalLectures'].'</td>
	<td style="width:50px;text-align:center;">
		<a class="btn btn-xs btn-info" href="courses.php?id='.$_GET['id'].'&prgid='.$rowrelted['id_prg'].'&timing='.$rowrelted['timing'].'&semester='.$rowrelted['semester'].$secthref.'&view=Attendance&tpl=2"><i class="icon-zoom-in"></i></a></td>
</tr>';
//--------------------------------------------------
	}
echo ' 
...
//------------------------------------------------
if($_GET['id'] && $_GET['semester'] && $_GET['prgid'] && $_GET['timing'] && $_GET['tpl']) {
//------------------------------------------------
if(date('Y-m-d' , strtotime(cleanvars($_POST['dated']))) != date("Y-m-d")) { 
	echo '<div class="alert-box warning" style="width:85%;"><span>Notice: </span>You cannot add attendance because current date and your lecture date difference. </div>';
} else {
//------------------------------------------------
...
//------------------------------------------------
	$sqllmsdailyLecture  = $dblms->querylms("SELECT  d.id, days  
										FROM ".TIMETABLE_DETAILS." d  
										INNER JOIN ".TIMETABLE." t ON t.id = d.id_setup   
										INNER JOIN ".PROGRAMS." p ON p.prg_id = t.id_prg   
										WHERE t.id_campus = '".cleanvars($_SESSION['userlogininfo']['LOGINIDCOM'])."' 
										...
										AND d.id_curs = '".cleanvars($_GET['id'])."' 
										AND d.days = '".$dayname."' $secLecture 
										AND t.id_prg = '".cleanvars($_GET['prgid'])."' AND t.status =  '1'
										AND t.semester = '".cleanvars($_GET['semester'])."' 
										AND t.timing = '".cleanvars($_GET['timing'])."' ");
	$countLecture = mysqli_num_rows($sqllmsdailyLecture);
//------------------------------------------------
...
if($countLecture == 0) { 
//------------------------------------------------
	echo '<div class="alert-box warning"><span>Notice: </span><font style="color:red; font-weight:600;font-size:14px;">Today you have no lecture in this class.</font></div>';
//------------------------------------------------
} else if(mysqli_num_rows($sqllmscheck)>=$countLecture) { 
//------------------------------------------------
	if($countLecture == 1) { 
		$totallecture = '<font style="color:blue; font-weight:600; font-size:18px;">'.$countLecture.'</font> lecture';
	} else { 
		$totallecture = '<font style="color:blue; font-weight:600; font-size:18px;">'.$countLecture.'</font> lectures';
	}
//------------------------------------------------
	echo '<div class="alert-box warning"><span>Notice: </span>Today you have only '.$totallecture.' in this class.</div>';
//------------------------------------------------
} else { 
//------------------------------------------------
...
//------------------------------------------------
	echo '<div class="alert-box warning"><span>Notice: </span>Record could not be added due to missing some values.</div>';
//------------------------------------------------
}
}
//--------------------------------------
...
if(date('Y-m-d' , strtotime(cleanvars($_POST['dated']))) != date("Y-m-d")) { 
	echo '
		<div class="alert-box warning" style="width:85%;"><span>Notice: </span>You cannot update attendance because current date and your lecture date difference. </div>';
} else {
//------------------------------------------------
...
<div class="row">
	<div class="col-lg-12">
		<div class="tabs-sub-nav">
			<span class="pull-left"><h3  style="font-weight:700;">Attendance - <font style="color:blue;">'.get_theorypractical($_GET['tpl']).'</font> ('.strtoupper($valuepname['prg_code']).' - '.addOrdinalNumberSuffix($_GET['semester']).') '.get_programtiming($_GET['timing']).'</h3></span>
...
//--------------------------------------------------
	$sqllmsassign  = $dblms->querylms("SELECT *  
										...
										AND at.id_teacher = '".cleanvars($rowsstd['emply_id'])."' 
										AND at.theorypractical = '".cleanvars($_GET['tpl'])."' AND at.semester = '".cleanvars($_GET['semester'])."'
...
<tr>
	...
	<td style="width:80px; text-align:center;">
		<a class="btn btn-xs btn-info iframeModal" data-height="450" data-width="100%" data-toggle="modal" data-target="#modalIframe"  data-modal-window-title="<b>'.$rowsurs['curs_code'].' - '.$rowsurs['curs_name'].' (Lecture #: '.$rowassign['lectureno'].')</b>" data-src="include/Teachers/courses/attendanceview.php?id='.$rowassign['id'].'&present='.$valuepresent['ttoalpresent'].'&absent='.$valueabsent['ttoalabsent'].'&prgname='.$valuepname['prg_name'].'&timing='.$_GET['timing'].'&semester='.$_GET['semester'].'" href="#"><i class="icon-zoom-in"></i></a>';
