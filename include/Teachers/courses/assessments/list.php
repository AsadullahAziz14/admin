...
if(cleanvars($_SESSION['userlogininfo']['LOGINIDCOM']) == 1) { 
	...
	$addPrgSemester = "AND ex.id_prg = '".cleanvars($_GET['prgid'])."'
...
//--------------------Total Questions ----------------------------
	$sqllmstotalques  = $dblms->querylms("SELECT eq.id_exam, ex.date_attempt, ex.total_marks, ex.obtain_marks, ex.id_term, ex.published, ex.id_prg
										FROM ".QUIZ_EXAMSQUESTIONS." eq  
										INNER JOIN ".QUIZ_EXAMS." ex ON ex.id = eq.id_exam 
...
if($valuesques['id_exam']) {
	if($valuesques['date_attempt'] != '0000-00-00 00:00:00') {
		...
		if($valuesques['published'] == 1) {
			...
			$linkview	 = '<a class="btn btn-xs btn-success" href="courses.php?id='.$_GET['id'].'&prgid='.$_GET['prgid'].'&timing='.$_GET['timing'].'&semester='.$_GET['semester'].$secthref.'&view=Assessments&examid='.$valuesques['id_exam'].'"><i class="icon-eye-open"></i></a>  ';
		} else {
			
			$linkview	 = '<a class="btn btn-xs btn-success" href="courses.php?id='.$_GET['id'].'&prgid='.$_GET['prgid'].'&timing='.$_GET['timing'].'&semester='.$_GET['semester'].$secthref.'&view=Assessments&examid='.$valuesques['id_exam'].'&term='.$_GET['term'].'"><i class="icon-eye-open"></i></a> 
			<a class="btn btn-xs btn-info" href="courses.php?id='.$_GET['id'].'&prgid='.$_GET['prgid'].'&timing='.$_GET['timing'].'&semester='.$_GET['semester'].$secthref.'&view=Assessments&examid='.$valuesques['id_exam'].'&term='.$_GET['term'].'&edit"><i class="icon-pencil"></i></a> ';
