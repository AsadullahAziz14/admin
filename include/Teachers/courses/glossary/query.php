...
$sqllmschecker  = $dblms->querylms("SELECT id  
	...
	if(mysqli_num_rows($sqllmschecker)>0) { 
		...
		header("Location:courses.php?id=".$_GET['id']."&view=Glossary", true, 301);
		exit();
	} else { 
...
//--------------------------------------
	...
	$section 	= $splitted[3];
//--------------------------------------
	$sqllmsrel = $dblms->querylms("INSERT INTO ".COURSES_GLOSSARYPROGRAM." (
																		id_setup				, 
																		id_prg					, 
																		semester				,
																		section					,
...
//--------------------------------------
			...
			$requestedvars .= ")\n";
//--------------------------------------
		$logremarks = 'Add Glossary #: '.$iddglossary.' for Course: '.$rowsurs['curs_code'].'-'.$rowsurs['curs_name'];
...
//--------------------------------------
if(isset($_POST['changes_detailglossary'])) { 
//------------------------------------------------
$sqllmsglossary  = $dblms->querylms("UPDATE ".COURSES_GLOSSARY." SET  status	= '".cleanvars($_POST['status'])."'
...
//--------------------------------------
if(!empty(sizeof($_POST['idprg']))) {
$sqllmsdelte  = $dblms->querylms("DELETE FROM ".COURSES_GLOSSARYPROGRAM." WHERE id_setup = '".cleanvars($_POST['editid'])."'");
//--------------------------------------
	for($ichk=0; $ichk<count($_POST['idprg']); $ichk++){
//--------------------------------------
	$arr 		= $_POST['idprg'][$ichk];
...
//--------------------------------------
if(isset($_POST['import_glossary'])) { 
//------------------------------------------------
$checkbox = $_POST['glsaryarchive'];
for($i=0;$i<count($_POST['glsaryarchive']);$i++) {
	$del_id = $checkbox[$i];
...
//--------------------------------------
			...
			$requestedvars .= '"Detail:"'.'=>'.'"'.$valuearachive['detail'].'",'."\n";
			$requestedvars .= '"Course ID:"'.'=>'.'"'.$_GET['id'].'",'."\n";
			$requestedvars .= '"Course Code:"'.'=>'.'"'.$rowsurs['curs_code'].'",'."\n";
			$requestedvars .= '"Course Name:"'.'=>'.'"'.$rowsurs['curs_name'].'",'."\n";
			$requestedvars .= '"Emply ID:"'.'=>'.'"'.$rowsstd['emply_id'].'"';
//--------------------------------------
		$logremarks = 'Add Glossary #: '.$lessonid.' for Course: '.$rowsurs['curs_code'].'-'.$rowsurs['curs_name'];
		$sqllmslog  = $dblms->querylms("INSERT INTO ".LOGSTEACHER." (
															id_user										, 
															filename									, 
															action										,
