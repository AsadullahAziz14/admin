...
if(mysqli_num_rows($sqllmscursrelated) > 0) {
	...
	while($rowrelted = mysqli_fetch_array($sqllmscursrelated)) { 
		...
		if($valuesetup['id']) { 
			...
			$linkhref = '<a class="btn btn-xs btn-success iframeModal" data-height="450" data-width="100%" data-toggle="modal" data-target="#modalIframe"  data-modal-window-title="<b>FINAL TERM AWARD LIST '.$rowrelted['prg_name'].' '.addOrdinalNumberSuffix($rowrelted['semester']).' ('.$cursename.')</b>" data-src="finaltermawardlistview.php?id='.$valuesetup['id'].'&timing='.$rowrelted['timing'].'&curse='.$cursename.'" href="#"><i class="icon-zoom-in"></i></a> 
				<a class="btn btn-xs btn-information" href="finaltermawardprint.php?id='.$valuesetup['id'].'" target="_blank"><i class="icon-print"></i></a>';
			if($valuesetup['forward_to'] != 4) { 	
				$linkhref .= '<a class="btn btn-xs btn-info" href="courses.php?id='.$_GET['id'].'&prgid='.$rowrelted['id_prg'].'&timing='.$rowrelted['timing'].'&semester='.$rowrelted['semester'].$secthref.'&view=Practicalmarks"><i class="icon-pencil"></i></a>';
			}
		}  else { 
			$linkhref = '<a class="btn btn-xs btn-info" href="courses.php?id='.$_GET['id'].'&prgid='.$rowrelted['id_prg'].'&timing='.$rowrelted['timing'].'&semester='.$rowrelted['semester'].$secthref.'&view=Practicalmarks"><i class="icon-plus"></i> Add</a>';
			$forwardto = '';
		}
		...
		<tr>
			...
			<td style="width:70px; text-align:center;">'.($rowcurstds['Totalstds'] + $valuerepstd).'</td>
...
if(mysqli_num_rows($sqllmsCoursesAllocation) > 0) {
	...
	echo '
	<h3 style="margin-top:10px; font-weight:600; color:orangered;">Liberal Arts</h3>
	<div style="clear:both;"></div>
	<table class="footable table table-bordered table-hover">
	<thead>
	<tr>
	...
	while($valueCourseAllocation = mysqli_fetch_array($sqllmsCoursesAllocation)) { 
		...
		if($valueCourseAllocation['semester'] == 0){
			$sqllmsCoursesSemester  = $dblms->querylms("SELECT ca.semester 
															FROM ".LA_COURSES_ALLOCATION." ca
															WHERE  ca.id_campus = '".cleanvars($_SESSION['userlogininfo']['LOGINIDCOM'])."' 
															AND ca.academic_session = '".cleanvars($_SESSION['userlogininfo']['LOGINIDACADYEAR'])."'
															AND ca.status = '1' AND ca.id_curs = '".cleanvars($_GET['id'])."' 
															AND ca.section = '".$valueCourseAllocation['section']."' 
															AND ca.timing = '".$valueCourseAllocation['timing']."' 
															AND ca.is_deleted != '1'
															ORDER BY ca.section ASC LIMIT 1");
			$valueCourseSemester = mysqli_fetch_array($sqllmsCoursesSemester);

			$valueCourseAllocation['semester'] = $valueCourseSemester['semester'];
		}
		...
		$sqllmsLAsetup  = $dblms->querylms("SELECT f.id, f.exam_date, f.forward_to 
													...
													AND f.is_liberalarts = '1' AND f.theory_practical = '2' 
		...
		if($valueLAsetup['id']) {
			...
			if($valueLAsetup['forward_to'] != 4) { 
					$linkhrefLA .=  ' <a class="btn btn-xs btn-info" href="courses.php?id='.$_GET['id'].'&prgid=la&timing='.$valueCourseAllocation['timing'].'&semester='.$valueCourseAllocation['semester'].'&section='.$valueCourseAllocation['section'].'&view=Practicalmarks"><i class="icon-pencil"></i></a> ';
			}
			
		}  else { 
			$linkhrefLA = '<a class="btn btn-xs btn-info" href="courses.php?id='.$_GET['id'].'&prgid=la&timing='.$valueCourseAllocation['timing'].'&semester='.$valueCourseAllocation['semester'].'&section='.$valueCourseAllocation['section'].'&view=Practicalmarks"><i class="icon-plus"></i> Add</a>';
			$forwardto = '';
		}
