...
//-----------------------final term Result Submission-------------------------
	...
	if(($_GET['prgid'] !='la')) { 
		...
		$sqllmsstds  = $dblms->querylms("SELECT od.*, std.std_id, std.std_name, std.std_rollno, std.std_regno, std.std_liberalarts,
		...
		while($rowcurstds = mysqli_fetch_array($sqllmsstds)) { 
			...
			$cursstudents[] = array (
										...
										"std_regno"		=> $rowcurstds['std_regno']		,
										"id_prg"		=> $rowcurstds['id_prg']		,
										"std_liberalarts"		=> $rowcurstds['std_liberalarts']		,
										"prg_name"		=> ''							,
										"rep_mig"		=> 0	
									);
...
foreach($cursstudents as $itemstd) { 
	...
	$sqllmspayable  = $dblms->querylms("SELECT 
											SUM(CASE WHEN total_amount != '0'  then total_amount end) as Totalpayable, 
											SUM(CASE WHEN total_amount != '0' then arrears end) as Totalpayarrears, 
											SUM(CASE WHEN paid_date != '0000-00-00'  then total_amount end) as Totalpaid, 
											SUM(CASE WHEN paid_date != '0000-00-00'  then arrears end) as Totalarrears
											FROM ".FEES." 
...
if($itemstd['std_liberalarts'] != 1) {
	$programSemester = " AND clp.id_prg  = '".cleanvars($itemstd['id_prg'])."' 
						 AND clp.semester  = '".cleanvars($itemstd['std_semester'])."'";
} else {
	$programSemester = '';
}

$queryAttendanceCount  = $dblms->querylms("SELECT COUNT(CASE WHEN clp.theorypractical = '2' then 1 else null end) totalPracticalLectures,
												COUNT(CASE WHEN clp.theorypractical = '2' AND dt.status = '2' then 1 else null end) totalPracticalPresent
												FROM ".COURSES_ATTENDANCE_DETAIL." dt
												INNER JOIN ".COURSES_ATTENDANCE." clp ON clp.id = dt.id_setup 
												INNER JOIN ".STUDENTS." std  ON std.std_id = dt.id_std  
												WHERE clp.id_campus = '".cleanvars($_SESSION['userlogininfo']['LOGINIDCOM'])."' 
												AND clp.academic_session = '".cleanvars($_SESSION['userlogininfo']['LOGINIDACADYEAR'])."'
												AND clp.id_curs = '".cleanvars($_GET['id'])."' 
												AND clp.timing = '".cleanvars($_GET['timing'])."'  
												$programSemester
												AND dt.id_std = '".cleanvars($itemstd['std_id'])."'
												ORDER BY clp.lectureno ASC");
    $valueAttendanceCount = mysqli_fetch_assoc($queryAttendanceCount);

	// $attendancePractical = 0;
	// if($valueAttendanceCount['totalPracticalLectures']>0) { 

	// 	if(round(($valueAttendanceCount['totalPracticalPresent']/$valueAttendanceCount['totalPracticalLectures']) * 100)>0) { 
			 $attendancePractical = round(($valueAttendanceCount['totalPracticalPresent']/$valueAttendanceCount['totalPracticalLectures']) * 100);
	// 	} else { 
	// 		$attendancePractical = 0;
	// 	}
...
if($attendancePractical >=$attendancereq || $valueclearance['allowed'] == 1) {
