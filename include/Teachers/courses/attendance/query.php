...
if(isset($_POST['submit_attendance'])) {
	...
	/*
	$threeDaysBackDate = date('Y-m-d', strtotime("-3 days"));
	if(date('Y-m-d', strtotime(cleanvars($_POST['dated']))) < $threeDaysBackDate){

		$_SESSION['msg']['status'] = '<div class="alert-box warning"><span>Warning: </span>Previous days attendance can not be marked.</div>';
	...
	*/
		...
		if(isset($_GET['id']) && isset($_GET['semester']) && isset($_GET['prgid']) && isset($_GET['timing']) && isset($_GET['tpl'])) { 
			...
			if(mysqli_num_rows($sqllmschecklecture)>0) { 
				...
				$queryInsertAttendance = $dblms->querylms("INSERT INTO ".COURSES_ATTENDANCE." (
																					lectureno							, 
																					theorypractical						,
																					id_curs								,
																					semester							,  
			...
			$_SESSION['msg']['status'] = '<div class="alert-box warning"><span>Notice: </span>Record could not be added due to some missing values.</div>';
			header("Location:".$hrefredirect."", true, 301);
			exit();
		}
	//}
